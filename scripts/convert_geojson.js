// Script pour extraire et convertir les pays avec vraies frontières
// Utilise Natural Earth 50m pour inclure les micro-états
const fs = require('fs');

const raw = JSON.parse(fs.readFileSync('src/assets/countries_raw_50m.json', 'utf8'));

// Liste des pays à inclure avec leurs codes ISO2
const countryMap = {
    // EUROPE
...
for (const f of raw.features) {
    ...
    // Extraction spécifique de la Guyane française (GF) de l'entité France (FRA)
    if (iso3 === 'FRA' && geom.type === 'MultiPolygon') {
        const guianaIndex = geom.coordinates.findIndex(poly => {
            const pt = poly[0][0]; // Premier point du premier anneau
            // La Guyane est autour de Lon -53, Lat 4
        ...
        if (guianaIndex !== -1) {
            ...
            if (!found.has('GF')) {
                features.push({
                    ...
                    geometry: {
                        type: "Polygon",
                        coordinates: simplifyCoords(guianaPoly, 2)
                    }
                });
                found.add('GF');
            }
            // Retirer la Guyane du multi-polygone France Métropolitaine
    ...
    features.push({
        ...
        geometry: {
            type: geom.type,
            coordinates: simplifyCoords(geom.coordinates, 2)
        }
    });
}

const output = `// GeoJSON - Monde complet (Natural Earth 110m)
// ${features.length} pays avec codes ISO2

export default ${JSON.stringify({ type: "FeatureCollection", features }, null, 2)};
`;

fs.writeFileSync('src/assets/world_simplified.ts', output);
console.log(`Exported ${features.length} countries`);
